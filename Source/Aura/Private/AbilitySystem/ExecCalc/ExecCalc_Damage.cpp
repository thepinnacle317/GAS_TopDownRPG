...
struct AuraDamageStatics
{
	...
	// Used to map a GameplayTag to the CapturedAttribute
	...
	FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	// Get Damage SetByCaller Magnitude from looping through the map of Damage Resistances
	float Damage = 0.f;
	for (const TTuple<FGameplayTag, FGameplayTag>& Pair : FAuraGameplayTags::Get().DamageTypesToResistances)
	{
		const FGameplayTag DamageTypeTag = Pair.Key;
		const FGameplayTag ResistanceTag = Pair.Value;
		checkf(AuraDamageStatics().TagsToCaptureDefs.Contains(ResistanceTag), TEXT("TagsToCaptureDefs doesn't containg Tag: [%s] in ExecCalc_Damage"), *ResistanceTag.ToString());

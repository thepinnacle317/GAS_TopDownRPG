...
void UAuraAbilitySystemComponent::AbilityInputTagHeld(const FGameplayTag& InputTag)
{
	...
	// Check if we have any activateable abilities related to the input tag and try to activate it if there is a matching tag.
...
{
	if (!InputTag.IsValid()) return;
	// Check for a matching tag for ability to input.  Then tell the ability that the input has been released.
	for(FGameplayAbilitySpec& AbilitySpec : GetActivatableAbilities())
	{
		if (AbilitySpec.DynamicAbilityTags.HasTagExact(InputTag))
		{
			// Sets a bool if the input was released
